...
-- Crear tabla de metadatos de adjuntos en la base de datos (public.attachments)
...
CREATE POLICY "attachments_db_public_read" ON public.attachments FOR SELECT USING (true);

-- Sólo el autor del contenido puede asociarle adjuntos. Los enunciados
-- (problems) sólo pueden ser gestionados por staff.
CREATE POLICY "attachments_db_insert_own_content_or_staff" ON public.attachments
FOR INSERT TO authenticated
WITH CHECK (
  public.is_staff()
  OR EXISTS (SELECT 1 FROM public.topics t WHERE t.id = topic_id AND t.author_id = auth.uid())
  OR EXISTS (SELECT 1 FROM public.replies r WHERE r.id = reply_id AND r.author_id = auth.uid())
);

-- Sólo el autor del contenido al que pertenece el adjunto puede borrarlo.
CREATE POLICY "attachments_db_delete_own_content_or_staff" ON public.attachments
