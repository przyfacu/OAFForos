...
async function home(){
  ...
  main.innerHTML=`<section class="hero"><div class="eyebrow">Comunidad argentina de física olímpica</div><h1>Espacio de discusión de física</h1><p>Un lugar para discutir problemas, compartir ideas y construir un archivo vivo de competencias de física.</p><a class="button" href="#foro">Explorar el foro</a></section><section class="section"><div class="section-head"><div><div class="eyebrow">Espacios de discusión</div><h2>Entrá a los foros</h2></div><a class="text-link" href="#foro">Ver el foro →</a></div><div class="grid cards-3">${categories.slice(0,3).map(c=>`<a class="card" href="#foro/${c.id}"><span class="category-icon">${getIcon(c.id)}</span><h3>${esc(c.title)}</h3><p>${esc(c.description)}</p></a>`).join("")}</div></section><section class="section two-column"><div><div class="eyebrow">Conversaciones recientes</div><h2>Últimos temas</h2><div class="feed">${topics.slice(0,4).map(topicRow).join("")}</div></div><aside class="sidebar-box"><h3>El archivo de problemas</h3><p>Recorré competencias, ediciones y niveles. Cada problema abre una conversación para resolverlo en comunidad.</p><a class="text-link" href="#archivo">Ir al archivo →</a></aside></section>
